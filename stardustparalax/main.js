...
(() => {
  ...
  function resize() {
    ...
    cx = cw * 0.5;
    cy = ch * 0.5;
    ph = cy * 0.4; // получаем 1/4 от центра высоты
    pw = cx * 0.4; // получаем 1/4 от центра ширины
  }
  resize();
  ...
  class Orb {
    constructor() {
      this.size = Math.random() * 5 + 2; // радиус круга
      this.angle = Math.random() * 360; // 0 до 360 градусов
      this.radius =
    ...
    refresh() {
      let radian = (this.angle * Math.PI) / 180; //переводим в радианы
      ...
      let distansToCenter = Math.hypot(x - cx, y - cy);
      let distansToMouse = Math.hypot(x - mx, y - my);

      let optic = sin * this.size * this.impact * 0.7;
      let mEffect = distansToMouse <= 50 ? (1 - distansToMouse / 50) * 25 : 0;
      ...
      if (distansToCenter > ph - 1 || sin > 0) {
        ctx.strokeStyle = ctx.fillStyle = color;
        ctx.beginPath();
        ctx.arc(x, y, size, 0, 2 * Math.PI);
        distansToMouse <= 50 ? ctx.stroke() : ctx.fill();
      }
